	...
	$.fn.formToWizard = function(options) {
		...
		this.each(function() {
			...
			steps.each(function(i) {
				...
				} else if (i == count - 2) { //second last step
					//bind show prev element
					createButton(i, i-1, "Prev", element);
					createButton(i, i+1, "Finish", element, 'last');
		 ...
		 */
		function createButton(current, target, text, element, css){
      ...
      if (typeof(css) == "undefined") {
        css = '';
			...
			$("<a />").text(text).addClass("form-wizard-button").addClass(css).appendTo(parent).attr("href", "#").click(function(e){
        var invalid;
				e.preventDefault();
